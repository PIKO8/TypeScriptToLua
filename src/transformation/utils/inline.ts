            ...
            if (ts.isCallExpression(node)) {
                ...
                if (ts.isIdentifier(callee) && lambdaParamMap.has(callee.text)) {
                    ...
                    const transformedArgs = ts.factory.createNodeArray(
                        ts.visitNodes(node.arguments, visit).filter(ts.isExpression)
                    );
                    ...
                    return substituteLambdaCall(otherCtx, info, {
                        ...node,
                        arguments: transformedArgs,
                    } satisfies ts.CallExpression);
                }
...
function expandStatement(
    ...
    if (ts.isIfStatement(stmt)) {
        const thenBlock = ts.isBlock(stmt.thenStatement)
            ...
            : stmt.thenStatement;
        const elseBlock =
            stmt.elseStatement && ts.isBlock(stmt.elseStatement)
...
function transformStatementForGoto(
    ...
    if (ts.isReturnStatement(stmt)) {
        ...
        if (lastResult && lastResult === stmt) {
            return {
                statements: [ts.factory.createExpressionStatement(ts.factory.createAssignment(resultVar, expr))],
                hasGoto: false,
            };
        }
        return {
    ...
    if (ts.isIfStatement(stmt)) {
        ...
        const thenBlock =
            thenResult.statements.length === 1 && !ts.isBlock(stmt.thenStatement)
                ? thenResult.statements[0]
                : ts.factory.createBlock(thenResult.statements, true);

        const elseBlock =
            elseResult &&
            (elseResult.statements.length === 1 && !ts.isBlock(stmt.elseStatement!)
                ? elseResult.statements[0]
                : elseResult
                ? ts.factory.createBlock(elseResult.statements, true)
                : undefined);
    ...
    if (ts.isForStatement(stmt)) {
        ...
        const newBody =
            bodyResult.statements.length === 1 && !ts.isBlock(stmt.statement)
    ...
    if (ts.isForOfStatement(stmt)) {
        ...
        const newBody =
            bodyResult.statements.length === 1 && !ts.isBlock(stmt.statement)
                ? bodyResult.statements[0]
                : ts.factory.createBlock(bodyResult.statements, true);
        const newForOf = ts.factory.createForOfStatement(
            stmt.awaitModifier,
            stmt.initializer,
            stmt.expression,
            newBody
...
function finishBuildDoBlockForLambdaLuaOlderThan52(
    resultVar: ts.Identifier,
    withParams: ts.Block,
        ...
        const visit = (node: ts.Node): ts.Node => {
            ...
            if (ts.isIfStatement(node)) {
                ...
                if (ts.isBlock(node.thenStatement)) {
                    const outStmts = ts
                        .visitNodes(node.thenStatement.statements, visit)
                ...
                if (node.elseStatement) {
                    if (ts.isBlock(node.elseStatement)) {
                        const outStmts = ts
                            .visitNodes(node.elseStatement.statements, visit)
                            .map(n => (ts.isExpression(n) ? ts.factory.createExpressionStatement(n) : n))
                            .filter(ts.isStatement);
    ...
    const finalStatements = [doneFlagDecl, ...guardedBody.statements];
...
function buildDoBlockForLambda(
    ...
    const { statements: withGoto, hasGoto } = transformBlockForGoto(
        withParams,
        resultVar,
        endLabel,
        isLastStmtIsReturn ? lastStmt : undefined
    );

    // Добавляем метку в конце
    let finalStatements = ts.factory.createNodeArray([...withGoto]);
    if (hasGoto) {
        finalStatements = ts.factory.createNodeArray([...finalStatements, createLabelMarker(endLabel)]);
    }
...
function createGotoMarker(label: string): ts.Statement {
    return ts.factory.createExpressionStatement(
        ts.factory.createCallExpression(ts.factory.createIdentifier("__TSLUA_goto"), undefined, [
...
function createLabelMarker(label: string): ts.Statement {
    return ts.factory.createExpressionStatement(
        ts.factory.createCallExpression(ts.factory.createIdentifier("__TSLUA_label"), undefined, [
            ts.factory.createStringLiteral(label),
        ])
    );
}
...
export function prepareInlineBody(
    ...
    (context as any as InlineContextExtend).__inlineDepth =
        ((context as any as InlineContextExtend).__inlineDepth ?? 0) + 1;
    try {
        ...
        throw new WrappedError(
            `Inline error, node: ${inlineInfo.node?.name?.text}, args: ${args.map(a => a.getText()).join(", ")}`,
            { cause: e }
        );
    } finally {
        (context as any as InlineContextExtend).__inlineDepth--;
...
export function embedInlineResult(
    ...
    if (paramAndBodyStmts.length === 0) {
        ...
        if (target) {
            if (target.vars.length > 1) {
                // Деструктуризация
                const stmt = lua.createAssignmentStatement(target.vars, hasMulti ? returnExprs : [returnExprs[0]]);
                if (context.options.inlineGenerateComment) {
    ...
    if (!target && !isReturnContext) {
        const isVoid = returnExprs.length === 0 || (returnExprs.length === 1 && lua.isNilLiteral(returnExprs[0]));
...
export function isInlineFunctionCandidate(context: TransformationContext, node: SignatureDeclaration): boolean {
